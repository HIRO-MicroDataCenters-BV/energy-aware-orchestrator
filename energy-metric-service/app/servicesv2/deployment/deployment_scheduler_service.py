"""
Deployment Scheduler Service
Processes pending deployment requests based on energy availability
"""

import logging
from datetime import datetime
from sqlalchemy.ext.asyncio import AsyncSession
from typing import List

from app.models.ApplicationDeployment import ApplicationDeployment, RequestStatus, WorkloadType
from app.repositories.application_deployment_repository import ApplicationDeploymentRepository
from app.repositories.application_definition_repository import ApplicationDefinitionRepository
from app.servicesv2.deployment.deployment_service import DeploymentHelperService
from app.servicesv2.deployment.kubernetes_native_deployment_service import KubernetesNativeDeploymentService
from app.servicesv2.deployment.helm_deployment_service import HelmDeploymentService
from app.servicesv2.deployment.custom_resource_deployment_service import CustomResourceDeploymentService
from app.db.database import AsyncSessionLocal

logger = logging.getLogger(__name__)


class DeploymentSchedulerService:
    """Service to process pending deployments based on energy availability"""

    def __init__(self):
        # Helper service for energy checks and common utilities
        self.deployment_helper = DeploymentHelperService()

        # Direct access to specialized deployment services
        self.kubernetes_native_service = KubernetesNativeDeploymentService()
        self.helm_service = HelmDeploymentService()
        self.custom_resource_service = CustomResourceDeploymentService()

    async def process_pending_deployments(self) -> int:
        """
        Process all pending deployment requests (status = 'Created' or 'Scheduled').

        Returns:
            Number of deployments processed
        """
        processed_count = 0

        async with AsyncSessionLocal() as db:
            try:
                deployment_repo = ApplicationDeploymentRepository(db)
                app_definition_repo = ApplicationDefinitionRepository(db)

                # Get all pending deployments (status = 'Created' or 'Scheduled')
                pending_deployments = await deployment_repo.get_by_statuses(
                    statuses=[RequestStatus.CREATED.value, RequestStatus.SCHEDULE.value],
                    limit=100
                )

                if not pending_deployments:
                    logger.debug("No pending deployments found")
                    return 0

                logger.info(f"Found {len(pending_deployments)} pending deployment(s) to process")

                for deployment in pending_deployments:
                    try:
                        await self._process_single_deployment(
                            deployment,
                            app_definition_repo,
                            deployment_repo,
                            db
                        )
                        processed_count += 1
                    except Exception as e:
                        logger.error(f"Failed to process deployment {deployment.id}: {e}")
                        # Mark as failed
                        await deployment_repo.update_deployment_request(
                            request_id=deployment.id,
                            status=RequestStatus.FAILED.value,
                            error_message=f"Scheduler processing error: {str(e)}"
                        )
                        await db.commit()

                logger.info(f"Processed {processed_count} pending deployment(s)")
                return processed_count

            except Exception as e:
                logger.error(f"Error in process_pending_deployments: {e}")
                await db.rollback()
                raise

    async def _process_single_deployment(
            self,
            deployment: ApplicationDeployment,
            app_definition_repo: ApplicationDefinitionRepository,
            deployment_repo: ApplicationDeploymentRepository,
            db: AsyncSession
    ):
        """
        Process a single pending deployment request.

        Args:
            deployment: The deployment request to process
            app_definition_repo: Repository for app definitions
            deployment_repo: Repository for deployments
            db: Database session
        """
        logger.info(f"Processing deployment request {deployment.id}")

        # Get app definition
        app_definition = await app_definition_repo.get_by_id(deployment.app_definition_id)
        if not app_definition:
            logger.error(f"App definition {deployment.app_definition_id} not found for deployment {deployment.id}")
            await deployment_repo.update_deployment_request(
                request_id=deployment.id,
                status=RequestStatus.FAILED.value,
                error_message="App definition not found"
            )
            await db.commit()
            return

        logger.info(
            f"Processing deployment for app '{app_definition.name}' (workload type: {app_definition.workload_type}, deployment type: {app_definition.deployment_type})")

        # Delegate directly to appropriate deployment service based on deployment type
        if app_definition.deployment_type == 'custom':
            logger.info(f"Delegating to CustomResourceDeploymentService for '{app_definition.name}'")
            await self._deploy_custom_resource(
                deployment=deployment,
                app_definition=app_definition,
                deployment_repo=deployment_repo,
                db=db
            )
        elif app_definition.deployment_type == 'kubernetes':
            logger.info(f"Delegating to KubernetesNativeDeploymentService for '{app_definition.name}'")
            await self._deploy_kubernetes_native(
                deployment=deployment,
                app_definition=app_definition,
                deployment_repo=deployment_repo,
                db=db
            )
        elif app_definition.deployment_type == 'helm':
            logger.info(f"Delegating to HelmDeploymentService for '{app_definition.name}'")
            await self._deploy_helm_chart(
                deployment=deployment,
                app_definition=app_definition,
                deployment_repo=deployment_repo,
                db=db
            )
        else:
            error_msg = f"Unsupported deployment type: {app_definition.deployment_type}"
            logger.error(error_msg)
            await deployment_repo.update_deployment_request(
                request_id=deployment.id,
                status=RequestStatus.FAILED.value,
                error_message=error_msg
            )
            await db.commit()
            return

        # Commit the changes
        await db.commit()

        # Get updated deployment to log final status
        updated_deployment = await deployment_repo.get_by_id(deployment.id)
        if updated_deployment:
            logger.info(f"Updated deployment {deployment.id} with status: {updated_deployment.status}")

    async def _deploy_custom_resource(
            self,
            deployment: ApplicationDeployment,
            app_definition,
            deployment_repo: ApplicationDeploymentRepository,
            db: AsyncSession
    ):
        """
        Deploy custom resource directly without energy checks.

        Custom resources are applied directly as they typically represent
        orchestration configurations rather than workloads.

        Args:
            deployment: The deployment request
            app_definition: The app definition containing the custom resource manifest
            deployment_repo: Repository for deployments
            db: Database session
        """
        logger.info(f"Deploying custom resource '{app_definition.name}' - bypassing energy checks")

        try:
            # Add custom labels
            custom_labels = {
                "app-name": app_definition.name,
                "workload-type": app_definition.workload_type,
                "deployment-type": "custom",
                "deployed-by": "energy-scheduler"
            }

            logger.info(f"Applying custom resource to namespace '{app_definition.namespace}'")

            # Deploy directly using custom resource service
            deployment_result = await self.custom_resource_service.deploy_custom_resource(
                manifest=app_definition.manifest,
                namespace=app_definition.namespace,
                custom_labels=custom_labels
            )

            if deployment_result["success"]:
                await deployment_repo.update_deployment_request(
                    request_id=deployment.id,
                    status=RequestStatus.DEPLOYED.value,
                    deployed_at=datetime.utcnow(),
                    error_message=''
                )
                logger.info(f"Successfully deployed custom resource '{app_definition.name}'")
                logger.info(f"Deployed resources: {deployment_result.get('deployed_resources', [])}")
            else:
                await deployment_repo.update_deployment_request(
                    request_id=deployment.id,
                    status=RequestStatus.FAILED.value,
                    error_message=deployment_result.get("error", "Custom resource deployment failed")
                )
                logger.error(f"Custom resource deployment failed: {deployment_result.get('error', 'Unknown error')}")

        except Exception as e:
            await deployment_repo.update_deployment_request(
                request_id=deployment.id,
                status=RequestStatus.FAILED.value,
                error_message=f"Custom resource deployment exception: {str(e)}"
            )
            logger.error(f"Exception during custom resource deployment: {e}")

    async def _deploy_kubernetes_native(
            self,
            deployment: ApplicationDeployment,
            app_definition,
            deployment_repo: ApplicationDeploymentRepository,
            db: AsyncSession
    ):
        """
        Deploy native Kubernetes resources with energy checks.

        Args:
            deployment: The deployment request
            app_definition: The app definition containing Kubernetes manifest
            deployment_repo: Repository for deployments
            db: Database session
        """
        logger.info(f"Deploying Kubernetes native resources for '{app_definition.name}'")

        # Get estimated energy
        estimated_energy = deployment.estimated_energy_watts
        if not estimated_energy:
            estimated_energy = app_definition.estimated_energy_required
        if not estimated_energy:
            estimated_energy = await self.deployment_helper.estimate_deployment_energy(app_definition.manifest)

        # Check energy availability based on workload type
        if app_definition.workload_type == 'Critical':
            # Critical workloads bypass energy checks
            logger.info(f"Critical workload '{app_definition.name}' - bypassing energy check")
            energy_check = {"sufficient": True, "reason": "Critical workload - energy check bypassed"}
        else:
            # For Preferred and Optional workloads, check energy availability
            energy_check = await self.deployment_helper.check_energy_availability(
                required_energy_watts=estimated_energy,
                db_session=db
            )
            logger.info(f"Energy check for '{app_definition.name}': {energy_check}")

        # If sufficient energy, deploy
        if energy_check["sufficient"]:
            try:
                # Add custom labels
                custom_labels = {
                    "app-name": app_definition.name,
                    "workload-type": app_definition.workload_type,
                    "deployment-type": "kubernetes",
                    "deployed-by": "energy-scheduler"
                }

                logger.info(f"Deploying Kubernetes resources to namespace '{app_definition.namespace}'")

                # Deploy using Kubernetes native service
                deployment_result = await self.kubernetes_native_service.deploy_kubernetes_resources(
                    manifest=app_definition.manifest,
                    namespace=app_definition.namespace,
                    custom_labels=custom_labels
                )

                if deployment_result["success"]:
                    await deployment_repo.update_deployment_request(
                        request_id=deployment.id,
                        status=RequestStatus.DEPLOYED.value,
                        deployed_at=datetime.utcnow(),
                        error_message=''
                    )
                    logger.info(f"Successfully deployed Kubernetes resources '{app_definition.name}'")
                else:
                    await deployment_repo.update_deployment_request(
                        request_id=deployment.id,
                        error_message=deployment_result.get("error", "Deployment failed")
                    )
                    logger.error(f"Deployment failed: {deployment_result.get('error', 'Deployment failed')}")

            except Exception as e:
                await deployment_repo.update_deployment_request(
                    request_id=deployment.id,
                    status=RequestStatus.FAILED.value,
                    error_message=f"Deployment exception: {str(e)}"
                )
                logger.error(f"Exception during deployment: {e}")
        else:
            # Insufficient energy - keep as CREATED but update error message
            await deployment_repo.update_deployment_request(
                request_id=deployment.id,
                error_message=f"Waiting for sufficient energy: {energy_check['reason']}"
            )
            logger.info(f"Deployment '{app_definition.name}' waiting for energy: {energy_check['reason']}")

    async def _deploy_helm_chart(
            self,
            deployment: ApplicationDeployment,
            app_definition,
            deployment_repo: ApplicationDeploymentRepository,
            db: AsyncSession
    ):
        """
        Deploy Helm chart with energy checks.

        Args:
            deployment: The deployment request
            app_definition: The app definition containing Helm chart configuration
            deployment_repo: Repository for deployments
            db: Database session
        """
        logger.info(f"Deploying Helm chart for '{app_definition.name}'")

        # Get estimated energy
        estimated_energy = deployment.estimated_energy_watts
        if not estimated_energy:
            estimated_energy = app_definition.estimated_energy_required
        if not estimated_energy:
            # Default estimate for Helm charts
            estimated_energy = 50.0
            logger.warning(f"No energy estimate available for Helm chart, using default: {estimated_energy}W")

        # Check energy availability based on workload type
        if app_definition.workload_type == 'Critical':
            # Critical workloads bypass energy checks
            logger.info(f"Critical workload '{app_definition.name}' - bypassing energy check")
            energy_check = {"sufficient": True, "reason": "Critical workload - energy check bypassed"}
        else:
            # For Preferred and Optional workloads, check energy availability
            energy_check = await self.deployment_helper.check_energy_availability(
                required_energy_watts=estimated_energy,
                db_session=db
            )
            logger.info(f"Energy check for '{app_definition.name}': {energy_check}")

        # If sufficient energy, deploy
        if energy_check["sufficient"]:
            try:
                # Add custom labels
                custom_labels = {
                    "app-name": app_definition.name,
                    "workload-type": app_definition.workload_type,
                    "deployment-type": "helm",
                    "deployed-by": "energy-scheduler"
                }

                logger.info(f"Deploying Helm chart to namespace '{app_definition.namespace}'")

                # Deploy using Helm service
                deployment_result = await self.helm_service.deploy_helm_chart(
                    manifest=app_definition.manifest,
                    namespace=app_definition.namespace,
                    custom_labels=custom_labels
                )

                if deployment_result["success"]:
                    await deployment_repo.update_deployment_request(
                        request_id=deployment.id,
                        status=RequestStatus.DEPLOYED.value,
                        deployed_at=datetime.utcnow(),
                        error_message=''
                    )
                    logger.info(f"Successfully deployed Helm chart '{app_definition.name}'")
                else:
                    await deployment_repo.update_deployment_request(
                        request_id=deployment.id,
                        error_message=deployment_result.get("error", "Helm deployment failed")
                    )
                    logger.error(f"Helm deployment failed: {deployment_result.get('error', 'Deployment failed')}")

            except Exception as e:
                await deployment_repo.update_deployment_request(
                    request_id=deployment.id,
                    status=RequestStatus.FAILED.value,
                    error_message=f"Helm deployment exception: {str(e)}"
                )
                logger.error(f"Exception during Helm deployment: {e}")
        else:
            # Insufficient energy - keep as CREATED but update error message
            await deployment_repo.update_deployment_request(
                request_id=deployment.id,
                error_message=f"Waiting for sufficient energy: {energy_check['reason']}"
            )
            logger.info(f"Helm deployment '{app_definition.name}' waiting for energy: {energy_check['reason']}")
