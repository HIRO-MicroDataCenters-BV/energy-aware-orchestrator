...
class KeplerNodeMetricsService:
    ...
    def _calculate_watts(self, metric_key: str, current_metrics: dict, previous_data: dict) -> float:
        ...
        # If no previous data, return 0.0 for first measurement
        # We cannot accurately calculate watts without a time delta
        if not previous_data or metric_key not in previous_data['metrics']:
            logging.debug(f"No previous data for {metric_key} - returning 0.0 for first measurement")
            return 0.0
