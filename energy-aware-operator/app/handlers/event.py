...
class EventHandler:
    ...
    def post_scheduling_completed(
        self, body: Dict[str, Any], schedule_result: Dict[str, Any]
    ) -> bool:
        """
        ...
        Args:
            body: The CR body
            schedule_result: The schedule result (dict format)

        Returns:
            True if successful
        """
        decision = schedule_result.get("decision", {})
        action = decision.get("action", "Scheduled")
        reason = decision.get("reason", "Schedule calculated")

        return self.post_event_safe(
            body, event_type="Normal", reason="Scheduled", message=f"{action}: {reason}"
        )
